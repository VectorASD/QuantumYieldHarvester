...
Nb = 4       # Number of columns in State (4 bytes)
Nk = 4       # Key length in 4-byte words
Nr = Nk + 6  # Number of encryption rounds
...
def check_mul_by():
    for name, value in globals().copy().items():
        if name.startswith("mul_by_"):
            print(f"~~~\n{name}: {value}")
            assert len(value) == 256 and len(set(value)) == 256  # no duplicates allowed!
...
def KeyExpansion(Key):
  ...
  # KeyS = [ord(S) for S in Key]
    if len(KeyS) < 4 * Nk:
        for i in range(4*Nk - len(KeyS)):
            KeyS.append(1)
    KeyC = [[] for i in range(4)]   
    for r in range(4):
        for c in range(Nk):
            KeyC[r].append(KeyS[r + 4 * c])
    for col in range(Nk, Nb*(Nr + 1)):
        if col % Nk == 0:
            ...
            for j in range(len(tmp)):
                tmp[j] = sbox[tmp[j]]
            for row in range(4):
                KeyC[row].append(KeyC[row][col - Nk]^tmp[row]^rcon[row][int(col/Nk - 1)])
        else:
            for row in range(4):
                KeyC[row].append(KeyC[row][col - Nk]^KeyC[row][col - 1])
...
# very very very old my api (2019 year)...
def Hex(Zn,Tr=True):
