"""
Source: https://github.com/pyrogram/tgcrypto/tree/master/tgcrypto

Note on using a pure-Python AES implementation instead of tgcrypto:

The official tgcrypto library is a C extension that provides fast AES
operations (IGE, CBC, CTR) for Telegram. However, installing it on
Windows from source requires Microsoft C++ Build Tools, which is not
always available or convenient. Even with the build tools installed,
there can be issues related to using multiple drives (e.g., C: and D:)
that interfere with compilation or linking. Therefore, relying on
prebuilt wheels from untrusted sources is not desirable from a
security standpoint.

To avoid these complications and keep the project self-contained and
easy to run anywhere, we use our own pure-Python implementation of the
required AES modes (IGE, CBC, etc.). This version was tested against
tgcrypto's reference outputs to ensure correctness. Performance is
more than adequate for the small payloads (credentials, cookies) we
encrypt here.

If you still want to use tgcrypto for higher performance, you can
install it separately and provide a fallback, but for this project
the pure-Python version is the default and recommended option.
"""
...
def aes_test2():
    ...
    E = time()
    print("my ige256:", B - A, "  tgcrypto ige256:", D - C)
    print("my cbc256:", B2 - A2, "  tgcrypto cbc256:", E - D)
    exit()

# aes_test2()


# ~~~ additional: TDF-reader ~~~
